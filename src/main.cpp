        ...
        if (event == "telemetry") {
          ...
          for (int i = 0; i < sensor_fusion.size(); i++) {
            ...
            if (check_d < (center_lane + 2) && check_d > (center_lane - 2)) {
              ...
              double cipv_dist = check_s - car_s;

              if ((check_s > car_s) &&
                  (cipv_dist < check_dist) &&
                  (cipv_dist < cipv_s)) {
                encroaching = true;

                cipv_s = check_s - car_s;

                // If car in range, initiate lane change
                if (cipv_s < check_dist) {
                  change_lane = true;
                  cipv_v = check_speed * 2.237;
                }
              }
            }
          }

          // Handle lane change condition
          if (change_lane) {
            if (lane > 0) {
              for (int i = 0; i < sensor_fusion.size(); i++) {
                double check_d = sensor_fusion[i][6];
                ...
                if (check_d < (left_lane + 2) &&
                  ...
                  // If the car is too close, do not change lanes
            ...
            // If not in the far right lane
            else if (lane < 2) {
              for (int i = 0; i < sensor_fusion.size(); i++) {
                double check_d = sensor_fusion[i][6];

                // Evaluate right lane
                if (check_d < (right_lane + 2) &&
                    check_d > (right_lane - 2)) {
                  double vx = sensor_fusion[i][3];
                  double vy = sensor_fusion[i][4];
                  double check_speed = sqrt(vx * vx + vy * vy);
                  double check_s = sensor_fusion[i][5];
            ...
            // Determine the lane to change to and set hte lane number
            // accordingly
          ...
          // In Frenet add 30m spaced points evenly ahead of the starting reference
          ...
          // Establish true coordinates for planner
          vector<double> next_x_vals;
          vector<double> next_y_vals;

          // Keep the last coordinates from the last calculated path
          for (int i = 0; i < previous_path_x.size(); i++) {
            next_x_vals.push_back(previous_path_x[i]);
            next_y_vals.push_back(previous_path_y[i]);
          }

          // Determine the target look ahead distance
